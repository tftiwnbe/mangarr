...
private const val FILE_APPENDER_NAME = "MangarrDefaultAppender"
...
fun debugLogsEnabled(config: Config): Boolean {
    val propertyKey = "mangarr.bridge.config.server.debugLogsEnabled"
    val override = System.getProperty(propertyKey)
    if (override != null) {
        return override.toBoolean()
    }

    if (!config.hasPath("server.debugLogsEnabled")) {
        return false
    }

    val configValue = config.getValue("server.debugLogsEnabled").unwrapped()
    return when (configValue) {
        is Boolean -> configValue
        else -> configValue.toString().toBoolean()
    }
}
