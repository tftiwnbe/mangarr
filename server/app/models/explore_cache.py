...
class ExploreCachePage(SQLModel, table=True):
    """Tracks cache freshness for one source page in one explore section."""
...
class ExploreCacheItem(SQLModel, table=True):
...
class ExploreTitleDetailsCache(SQLModel, table=True):
    """Cached title details fetched from a source title page."""

    __tablename__: ClassVar[Any] = "explore_title_details_cache"

    source_id: str = Field(primary_key=True)
    title_url: str = Field(primary_key=True)
    title: str
    status: int = 0
    thumbnail_url: str = ""
    artist: str | None = None
